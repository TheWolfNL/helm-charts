...
{{- define "thewolfnl.common.render.routes" -}}
  ...
  {{- range $name, $route := .Values.route }}
    {{- if $route.enabled -}}
      ...
      {{/* set defaults */}}
      {{- if and (not $routeValues.nameOverride) (ne $name (include "thewolfnl.common.lib.route.primary" $)) -}}
        {{- $_ := set $routeValues "nameOverride" $name -}}
      {{- end -}}

      {{- $_ := set $ "ObjectValues" (dict "route" $routeValues) -}}
      {{- include "thewolfnl.common.class.route" $ | nindent 0 -}}
