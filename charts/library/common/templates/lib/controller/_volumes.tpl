...
*/}}
{{- define "thewolfnl.common.lib.controller.volumes" -}}
{{- range $index, $persistence := .Values.persistence }}
{{- if $persistence.enabled }}
- name: {{ $index }}
  {{- if eq (default "pvc" $persistence.type) "pvc" }}
    {{- $pvcName := (include "thewolfnl.common.lib.chart.names.fullname" $) -}}
    ...
    {{- else -}}
      ...
      {{- if $persistence.nameOverride -}}
        {{- if not (eq $persistence.nameOverride "-") -}}
          {{- $pvcName = (printf "%s-%s" (include "thewolfnl.common.lib.chart.names.fullname" $) $persistence.nameOverride) -}}
        {{- end -}}
      {{- else -}}
        {{- $pvcName = (printf "%s-%s" (include "thewolfnl.common.lib.chart.names.fullname" $) $index) -}}
      {{- end -}}
    {{- end }}
