...
{{- if .Values.addons.vpn.enabled -}}
  ...
  {{- if and .Values.addons.vpn.configFile (not .Values.addons.vpn.configFileSecret) }}
    {{- $secret := include "thewolfnl.common.addon.vpn.secret" . -}}
    {{- if $secret -}}
  ...
  {{- $scriptVolume := include "thewolfnl.common.addon.vpn.scriptsVolumeSpec" . | fromYaml -}}
  {{- if $scriptVolume -}}
  ...
  {{- $configVolume := include "thewolfnl.common.addon.vpn.configVolumeSpec" . | fromYaml }}
  {{ if $configVolume -}}
  ...
  {{- $networkpolicy := include "thewolfnl.common.addon.vpn.networkpolicy" . -}}
  {{- if $networkpolicy -}}
