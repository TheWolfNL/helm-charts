...
*/}}
{{- define "thewolfnl.common.addon.codeserver" -}}
  {{- if .Values.addons.codeserver.enabled -}}
    {{/* Append the code-server container to the sidecars */}}
    {{- $container := include "thewolfnl.common.addon.codeserver.container" . | fromYaml -}}
    {{- if $container -}}
    ...
    {{- if or .Values.addons.codeserver.git.deployKey .Values.addons.codeserver.git.deployKeyBase64 -}}
      {{- $deployKeySecret := include "thewolfnl.common.addon.codeserver.deployKeySecret" . -}}
      {{- if $deployKeySecret -}}
    ...
    {{- if or .Values.addons.codeserver.git.deployKey .Values.addons.codeserver.git.deployKeyBase64 .Values.addons.codeserver.git.deployKeySecret }}
      {{- $volume := include "thewolfnl.common.addon.codeserver.deployKeyVolumeSpec" . | fromYaml -}}
      {{- if $volume -}}
    ...
    {{- if .Values.addons.codeserver.service.enabled -}}
      ...
      {{- include "thewolfnl.common.class.service" $ -}}
      {{- $_ := unset $.ObjectValues "service" -}}
    {{- end -}}

    {{/* Add the code-server ingress */}}
    {{- $svcName := printf "%v-addon-codeserver" (include "thewolfnl.common.lib.chart.names.fullname" .) -}}
