...
*/}}
{{- define "thewolfnl.common.class.service" -}}
...
{{- $serviceName := include "thewolfnl.common.lib.chart.names.fullname" . -}}
{{- if and (hasKey $values "nameOverride") $values.nameOverride -}}
  {{- $serviceName = printf "%v-%v" $serviceName $values.nameOverride -}}
{{ end -}}
{{- $svcType := $values.type | default "" -}}
{{- $enabledPorts := include "thewolfnl.common.lib.service.enabledPorts" (dict "serviceName" $serviceName "values" $values) | fromYaml }}
{{- $primaryPort := get $values.ports (include "thewolfnl.common.lib.service.primaryPort" (dict "values" $values)) }}
---
apiVersion: v1
kind: Service
metadata:
  ...
  {{- range $name, $port := $enabledPorts }}
    - port: {{ $port.port }}
      ...
      nodePort: {{ $port.nodePort }}
        {{ end }}
      {{- end -}}
  {{- with (merge ($values.extraSelectorLabels | default dict) (include "thewolfnl.common.lib.metadata.selectorLabels" . | fromYaml)) }}
